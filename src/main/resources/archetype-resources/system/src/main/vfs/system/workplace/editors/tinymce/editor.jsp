...
function setupTinyMCE(editor) {
   if (tinyMCE.isWebKit) {
      ...
      editor.onInit.add(function() {
         ...
         var modElem = ${symbol_dollar}(baseElem).parents(".cmsTinyMCE").get(0);
         ${symbol_dollar}(modElem).removeClass("cmsTinyMCE");
         window.setTimeout(function() { ${symbol_dollar}(modElem).addClass("cmsTinyMCE"); } , 1);
      });
   }
}
...
if (elementSelection || elementLanguage) {
	...
	if (elementLanguage) {
		out.println("<td>" + wp.buildSelectElementLanguage("name=${symbol_escape}"" + CmsEditor.PARAM_ELEMENTLANGUAGE + "${symbol_escape}" width=${symbol_escape}"150${symbol_escape}" onchange=${symbol_escape}"buttonAction(3);${symbol_escape}"") + "</td>");
	...
	if (elementSelection) {
		out.println("<td>" + wp.buildSelectElementName("name=${symbol_escape}"" + CmsDefaultPageEditor.PARAM_ELEMENTNAME + "${symbol_escape}" width=${symbol_escape}"150${symbol_escape}" onchange=${symbol_escape}"buttonAction(3);${symbol_escape}"") + "</td>");
