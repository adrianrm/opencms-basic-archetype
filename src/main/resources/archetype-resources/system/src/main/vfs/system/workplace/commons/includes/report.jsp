...
// -->
    ...
    <%= wp.bodyStart("empty", "style=${symbol_escape}"background-color:ThreeDFace;${symbol_escape}" onLoad=${symbol_escape}"init();${symbol_escape}"") %><%= wp.bodyEnd() %>
    <%= wp.htmlEnd() %>
...
var cssStyle =
    "<style type='text/css'>${symbol_escape}n" +
    "body       { box-sizing: border-box; -moz-box-sizing: border-box; padding: 2px; margin: 0; color: ${symbol_pound}000000; background-color:${symbol_pound}ffffff; font-family: Verdana, Arial, Helvetica, sans-serif; font-size: 11px; }${symbol_escape}n" +
    "div.main   { box-sizing: border-box; -moz-box-sizing: border-box; color: ${symbol_pound}000000; white-space: nowrap; }${symbol_escape}n" +
    "span.head  { color: ${symbol_pound}000099; font-weight: bold; }${symbol_escape}n" +
    "span.note  { color: ${symbol_pound}666666; }${symbol_escape}n" +
    "span.ok    { color: ${symbol_pound}009900; }${symbol_escape}n" +
    "span.warn  { color: ${symbol_pound}990000; padding-left: 40px; }${symbol_escape}n" + 
    "span.err   { color: ${symbol_pound}990000; font-weight: bold; padding-left: 40px; }${symbol_escape}n" +   
    "span.throw { color: ${symbol_pound}990000; font-weight: bold; }${symbol_escape}n" +
    "span.link1 { color: ${symbol_pound}666666; }${symbol_escape}n" +
    "span.link2 { color: ${symbol_pound}666666; padding-left: 40px; }${symbol_escape}n" +    
    "span.link2 { color: ${symbol_pound}990000; }${symbol_escape}n" +    
    "</style>${symbol_escape}n";

var pageStartSimple =
    "<html>${symbol_escape}n<head>${symbol_escape}n" +
    "<meta HTTP-EQUIV='Content-Type' CONTENT='text/html; charset=<%= wp.getEncoding() %>'>${symbol_escape}n" + 
    "<link rel='stylesheet' type='text/css' href='<%= wp.getStyleUri("workplace.css") %>'>${symbol_escape}n" +
    cssStyle +
    "</head>${symbol_escape}n" +
    "<body style='background-color:Menu;'>${symbol_escape}n" +   
    "<div style='vertical-align:middle; height: 100%;'>${symbol_escape}n"+
    "<table border='0' style='vertical-align:middle; height: 100%;'>${symbol_escape}n" + 
    "<tr><td width='40' align='center' valign='middle'><img name='report_img' src='<%= CmsWorkplace.getSkinUri() %>commons/wait.gif' width='32' height='32' alt=''></td>${symbol_escape}n" + 
    "<td valign='middle'>";
    
var pageEndSimple = 
	"</td></tr>${symbol_escape}n" +
	"</table></div>${symbol_escape}n" +
	"</body>${symbol_escape}n</html>";    
...
var pageStartExtended =
	...
	cssStyle +
    "</head>${symbol_escape}n" +
    "<body style='overflow: auto;'>${symbol_escape}n" +  
    "<div class='main'>${symbol_escape}n";
    
var pageEndExtended = 
	"</div>${symbol_escape}n" +
	"</body>${symbol_escape}n" +
	"</html>${symbol_escape}n";                                
...
function update() {
    ...
    if (htmlText.length > size) {
        ...
        var pos = htmlText.indexOf("${symbol_escape}n"); 
        if (pos > 0) {
...
function updateReport() {
    if (reportType == "<%= I_CmsReport.REPORT_TYPE_SIMPLE %>") {
    	if (!isRunning && hasError) {
    		pageBody = 
    			pageStartSimple + 
    			"<span class='head'>" + lastHeadline + "</span><br>${symbol_escape}n" +
    			"<%= wp.key(Messages.RPT_ERROR_0) %> " + lastError + "<br>${symbol_escape}n" +
...
//-->
    ...
    <%= wp.bodyStart(null, "onLoad=${symbol_escape}"start();${symbol_escape}"") /*"dialog"*/%>
    <%= wp.dialogStart() %>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr><td width="100%" id="buttonrowcontinue" class="hide">
    <%= wp.dialogButtonsContinue("id=${symbol_escape}"contok${symbol_escape}" onclick=${symbol_escape}"updateWin.continueReport();${symbol_escape}"", "id=${symbol_escape}"contcancel${symbol_escape}"", "id=${symbol_escape}"contdetails${symbol_escape}"") %>
  </td></tr>
  <tr><td width="100%" id="buttonrowok" class="hide">
    <%= wp.dialogButtonsOkCancelDetails("id=${symbol_escape}"okclose${symbol_escape}"", "id=${symbol_escape}"okcancel${symbol_escape}"", "id=${symbol_escape}"details${symbol_escape}"") %>
  </td></tr>
  <tr><td>
