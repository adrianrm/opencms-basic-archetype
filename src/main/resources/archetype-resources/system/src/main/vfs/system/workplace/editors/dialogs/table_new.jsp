...
<%@ page import="org.opencms.workplace.*" %><%	
	...
	if ("edit".equals(titleType)) {
		...
		disabled = " disabled=${symbol_escape}"disabled${symbol_escape}" style=${symbol_escape}"background-color: Menu;${symbol_escape}"";
	} 
...
function setTBColor(arr) {
	if (ColorSelected == 1 && arr != -1 ) {
		document.main.TableColor.value = "${symbol_pound}" + arr;	      
...
function previewColor(inputId, previewId) {
	...
	if (colorValue == null) {
		document.getElementById(previewId).style.color = '${symbol_pound}000000';
		document.getElementById(previewId).style.backgroundColor = '${symbol_pound}FFFFFF';
	} else if (colorValue < 50000) {
		document.getElementById(previewId).style.color = '${symbol_pound}FFFFFF';	
		document.getElementById(previewId).style.backgroundColor = '${symbol_pound}' + colorValue;			
	} else {
		document.getElementById(previewId).style.color = '${symbol_pound}000000';
		document.getElementById(previewId).style.backgroundColor = '${symbol_pound}' + colorValue;		
	}
...
function checkField(elementId, checkFieldId) {
	...
	if (document.getElementById(elementId).value == "") {
		el.value = "";
		el.disabled = true;
		el.style.backgroundColor = "${symbol_pound}D0D0D0";
	} else {
		el.disabled = false;
		el.style.backgroundColor = "${symbol_pound}FFFFFF";
	}
}
...
<%= wp.bodyStart("dialog", "onload=${symbol_escape}"init();${symbol_escape}"") %>
...
<%= wp.dialogContentEnd() %>
<%= wp.dialogButtonsOkCancel(null, "onclick=${symbol_escape}"window.close();${symbol_escape}"") %>
</form>
