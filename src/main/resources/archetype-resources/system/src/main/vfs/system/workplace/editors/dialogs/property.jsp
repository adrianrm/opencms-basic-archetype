...
//////////////////// ACTION: save edited properties for the current resource type and close the popup window
	...
	if (wp.hasTemplateChanged()) {
		...
		additionalScript = "function closeAction() {${symbol_escape}n${symbol_escape}tthis.refreshOpener = true;${symbol_escape}n}${symbol_escape}nvar closeObj = new closeAction();${symbol_escape}nif (window.opener.popupCloseAction) {${symbol_escape}nwindow.opener.popupCloseAction(closeObj);${symbol_escape}n}";
	}
...
<%= wp.bodyStart("dialog", "onload=${symbol_escape}"window.setTimeout('doSet()',50);${symbol_escape}"") %>
...
<%= wp.dialogButtonsOkCancelAdvanced(null, null, "value=${symbol_escape}""+wp.key("button.advanced")+"${symbol_escape}" onclick=${symbol_escape}"submitAdvanced();${symbol_escape}"") %>
</form>
